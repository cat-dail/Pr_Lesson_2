...
def calculate_average(*nambers):
    calculate = 0
    try:
        for elem in range(len(nambers)):
            res = personal_sum(*nambers)
            calculate = res[0]/(len(*nambers) - res[1])
    except ZeroDivisionError:
        calculate = 0
    except TypeError:
        print(f'В nambers записан некорректный тип данных.')
        return
    return calculate
...
# строковый тип
print(f'Результат 2: {calculate_average([1, "Строка", 3, "Ещё Строка"])}')  # Учитываются только
# 1 и 3
print(f'Результат 3: {calculate_average(567)}')  # Передана не коллекция
print(f'Результат 4: {calculate_average([42, 15, 36, 13])}')  # Всё должно работать
